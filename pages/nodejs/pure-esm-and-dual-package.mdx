...
3. **출력(Dist Code)** : `tsc(컴파일러)` 는 `node.js` 좋아하는 cjs(commonjs)문법으로 싹 바꿔서 `dist/` 등의 출력 폴더에 `.js` 파일로 저장
4. **실행(Runtime)** : `node.js`는 `package.json`의 `"type": "commonjs"` 설정을 보고, 변환된 `require` 코드를 문제없이 실행함

- `tsconfig.json` 의 `"module"` 은 compile 결과를 위한 것
- `package.json` 의 `"type"` 은 이미 컴파일된 `.js`를 `commonjs` 형식으로 읽을 것
